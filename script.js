const profileImages = {
  ...
  const profile = {
    ...
    resumes: {
      frontend: "assets/resumes/frontend-resume.pdf",
      backend: "assets/resumes/backend-resume.pdf"
    }
  };
...
<div class="project-links">
  ...
  <a class="project-link" href="https://github.com/kookmin-sw/capstone-2024-19" target="_blank" rel="noreferrer">
    ...
    이력서: `이력서는 아래 파일에서 볼 수 있습니다.
<div class="project-links">
  <a class="project-link" href="${profile.resumes.frontend}" target="_blank" rel="noreferrer">
    <strong>프론트엔드 이력서</strong><span>PDF 파일 열기</span>
  </a>
  <a class="project-link" href="${profile.resumes.backend}" target="_blank" rel="noreferrer">
    <strong>백엔드 이력서</strong><span>PDF 파일 열기</span>
  </a>
</div>`
  };
  ...
  function resolveAnswer(question) {
    ...
    if (normalized.includes("이력서") || normalized.includes("resume") || normalized.includes("링크") || normalized.includes("사이트")) return ["이력서", true];
    return ["자기소개", false];
  }
  ...
  setTimeout(() => {
    appendMessage("안녕하세요! 궁금한 걸 물어보면 제 이력서와 프로젝트 정보를 기준으로 답해드릴게요.", false);
  }, 450);
